...
class CribbageCounter:
        ...
        while last < len(cards):
            if cards[first] == 0:
                ...
                if last - first > 2:
                    ...
                    points += subtotal
                first = last
        self.report(f"{points}pts from runs: {cards}")
        return points
    ...
    def count_fifteens(self, hand: list[Card]) -> int:
        ...
        self.report(f"{points}pts from fifteens: {fifteens}")
        return points
    ...
    def count_flushes(self, hand: list[Card]) -> int:
        ...
        for card in hand[2:]:
            if card.suit != hand[1].suit:
                self.report("no flush")
                return 0
        flush_size = len(hand)
        if hand[0].suit == hand[1].suit:
            flush_size += 1
        self.report(f"flush of {flush_size}")
        return flush_size
    ...
    def count_hand(self, hand: list[Card]) -> int:
        ...
        if len(hand) > 1 and len([card for card in hand[1:] if card.number == 11 and card.suit == hand[0].suit]) > 0:
            total += 1
            self.report("and one for the jack")
